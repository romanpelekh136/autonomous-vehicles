...
class TensorBoardReporter(BaseReporter):
    """Логує fitness, кількість видів та складність мережі в TensorBoard."""
    ...
    def post_evaluate(self, config, population, species, best_genome):
        ...
        if fitnesses:
            ...
            if best_genome.fitness is not None and best_genome.fitness > self.best_fitness:
                ...
                print(f"\n[+] New best NEAT model saved (fitness: {self.best_fitness:.2f})\n")
            
        self.generation += 1

def eval_genome(genome, config):
    """Оцінює один геном на кількох трасах. Повертає середню нагороду."""
